...
MIMETYPE_FILE = "mimetypes.csv" # loads all the static mimetypes! (csv is not from iana anymore)
...
DISABLE_MIMETYPES_READING = False # you want to use only dynamic ext reading if you want to disable this
...
def strip_fragment(u): return urldefrag(u)[0]
...
def build_base_url(raw):
    for scheme in ("https://", "http://"):
        try:
            r = scraper.get(scheme + raw, timeout=REQUEST_TIMEOUT)
...
visited, saved_paths, content_hashes = set(), set(), set()
site_fingerprints = {}
...
def enqueue(u): url_queue.put(normalize_url(u))
